...
const StudentFeedback = () => {
    ...
    const handleSubmit = async (event) => {
        ...
        try {
            ...
            const response = await axios.post(`${Backend_url}/api/v1/classes/give-student-feedback?id=${classId}&studentId=${studentId}`, {
                text: feedback
            }, {
                headers: {
            ...
            if (error.response?.status === 409) {
                setErrorMessage("Feedback already submitted for this student");
            } else {
    ...
    return (
        <div className="w-full flex flex-col items-center mt-10">
            <div className="w-10/12 md:w-2/3 lg:w-11/12 flex justify-start mb-4">
                <button onClick={goBack} className="text-cyan-800 border-cyan-800 outline bg-transparent px-4 py-2 font-bold rounded-md">
                    Back
                </button>
            </div>
            {successMessage && (
            ...
            <h1 className="text-2xl font-semibold text-cyan-900 mb-4">Give Feedback to the Student</h1>
