...
const ChatBox = () => {
  ...
  const getAllChatsWithMentor = async () => {
    try {
      ...
      const response = await axios.get(`${Backend_url}/api/v1/chats/get-all-chats-with-mentor?mentorId=${mentorId}`, {
        headers: {
      ...
      if (response.data && response.data.success) {
        const chats = response.data.data.map(chat => ({
          ...
          isYou: chat.receiverInfo._id === mentorId,
  ...
  const handleSendMessage = async () => {
    try {
      ...
      if (inputMessage.trim() !== '') {
        const response = await axios.post(`${Backend_url}/api/v1/chats/chat-with-mentor?mentorId=${mentorId}`, {
  ...
  return (
    <div className="flex flex-col h-screen">
      ...
      <div className="border-t bg-cyan-100 p-4">
        <div className="flex">
          <input
            type="text"
            value={inputMessage}
            ...
            className="flex-grow p-2 mr-2 border rounded focus:outline-none bg-cyan-50 "
          />
          <button
