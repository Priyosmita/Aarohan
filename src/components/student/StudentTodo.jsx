...
const navigate = useNavigate();
  ...
  const markAsDone = (classId) => {
    navigate(`/Student-Classroom/${classId}/Assignments`)
  };
  ...
  return (
    ...
    >
      <div className="container px-4 py-8 mx-20 justify-center">
        <h1 className="text-3xl font-normal mb-10 text-center text-cyan-950 ">
          Student Todo Lists
        </h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2=1 lg:grid-cols-1 gap-6 mx-10  w-11/12  ">
          {todos?.map((task) => (
            ...
            >
              <div>
                <h2 className="text-xl font-semibold mb-2 text-cyan-900">{task.description}</h2>
                <h2 className="text-cyan-700 text-xl mb-2 font-medium">{task.classname}</h2>
                <p className="text-cyan-700">Deadline: {new Date(task.deadline).toLocaleString()}</p>
                <p className="text-cyan-700">Fullmarks: {task.fullmarks}</p>
              </div>
                <button
                  onClick={() => markAsDone(task.classId)}
